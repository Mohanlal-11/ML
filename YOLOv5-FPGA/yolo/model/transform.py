...
class Transformer(nn.Module):
    def __init__(self, min_size, max_size, stride=32, mosaic=False):
        super().__init__()
        ...
        # self.flip_prob = 0
        self.mosaic = mosaic
        # print(f'in transfrom : {self.mosaic}')
    ...
    def forward(self, images, targets):
        ...
        images, targets, scale_factors, box_map = zip(*transformed)
        ...
        return images, targets, scale_factors, image_shapes, box_map
    ...
    def transforms(self, image, target):
        image, target, scale_factor, box_map = self.resize(image, target)
        
        if self.training:
            if random.random() < self.flip_prob:
                image, target["boxes"] = self.horizontal_flip(image, target["boxes"])
            
        return image, target, scale_factor, box_map
    ...
    def resize(self, image, target):
        ...
        image = transforms.Resize((self.min_size, self.max_size))(image)
        scale_factor = 1

        bw_map = self.min_size / orig_image_shape[1]
        bh_map = self.min_size / orig_image_shape[0]
        ...
        return image, target, scale_factor,(bh_map, bw_map)
