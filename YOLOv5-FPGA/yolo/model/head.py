...
class Head(nn.Module):
    ...
    def compute_loss(self, preds, targets):
        ...
        # print(f'gt labels : {gt_labels.min(), gt_labels.max()}')
        ...
        for pred, stride, wh in zip(preds, self.strides, self.anchors):
            ...
            if len(anchor_id) > 0:
                ...
                # print(f'gt_id : {gt_id}, {len(gt_id)}')
                image_id = image_ids[gt_id]
                # print(f'pred shape:{pred.shape}')
                # print(f'grid xy:{grid_xy[:, 1]}, {grid_xy[:, 0]}')
                
                grid_xy[:, 0] = grid_xy[:, 0].clamp(min=0, max=pred.shape[2] - 1)  # Ensure x-coordinates are within bounds
                grid_xy[:, 1] = grid_xy[:, 1].clamp(min=0, max=pred.shape[1] - 1)  # Ensure y-coordinates are within bounds

                pred_level = pred[image_id, grid_xy[:, 1], grid_xy[:, 0], anchor_id]
                # print(f'pred level : {pred_level.shape}')
