...
class ConcatBlock(nn.Module):
    def __init__(self, in_channels, out_channels, layer, fusion):
        ...
        #     nn.LeakyReLU(0.1015625, inplace=True),
        # )
        self.tail = nn.Sequential(
            nn.BatchNorm2d(2 * mid_channels, eps=bn_eps, momentum=bn_momentum),
            nn.Conv2d(2*mid_channels, 2*mid_channels, 1, bias=False).requires_grad_(False),
            nn.LeakyReLU(0.1015625, inplace=True),
        )
        
        self.conv = Conv(2 * mid_channels, out_channels, 1)
        
        wts = torch.zeros(2*mid_channels, 2*mid_channels, 1, 1)
        for i in range(2*mid_channels):
            wts[i, i, 0, 0] = 1

        self.tail[1].weight.copy_(wts)
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        Mc = torch.Hardsigmoid(avg_out + max_out)
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        Ms = torch.Hardsigmoid(self.conv(out))
