...
def show(images, targets=None, gt_box=None, classes=None, save=""):
    ...
    for i in range(len(images)):
        show_single(images[i], gt_box, targets[i] if targets else targets, classes, save[i])

    
def show_single(image, gt_box, target, classes, save):
    """
        ...
        if "boxes" in target:
            ...
            for i, b in enumerate(boxes):
                ...
                if gt_box is not None:
                    # print(gt_box)
                    gt_rect = patches.Rectangle(gt_box[:2], gt_box[2], gt_box[3], linewidth=3, edgecolor=(1,1,1), facecolor="none")
                ax.add_patch(rect)
                if gt_box is not None:
