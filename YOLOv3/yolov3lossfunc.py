...
from yolov3.utils import calculate_iou
...
class Yolov3Loss(nn.Module):
  ...
  def forward(self, predictions:torch.Tensor, labels:torch.Tensor, scale:str, device:torch.device):
    object_present = (labels[..., 4:5] == 1).float() #Here, if labels contains object then it will be equal to '1' and object_present will be all 1 of size [1,8,8,3,5+class] for large scale otherwise all 0.
    object_absence = (labels[..., 4:5] == 0).float() #Here, if labels doesn't contain object then it will be equal to '0' and object_absence will be all 1 of size [1,8,8,3,5+class] for large scale otherwise all 0.
    ...
    if scale == "large":
      no_obj_loss = self.noobj_loss_func(predictions[..., 4:5] * object_absence, labels[..., 4:5] * object_absence)

      pred_box_center = self.sigmoid(predictions[..., 0:2])
      pred_box_wh = large_scale_anchor * torch.exp(predictions[..., 2:4])# Here, when model predicts the output its has bounding boxes whose center are normalized [0,1] and width, height are relative to grid size and in its line number 49 we can see it is again divided by anchor and took log so here it is raised to exp and multiplied by anchor an for center the ground truth is normalize i.e. between [0,1] so took sigmoid.
      predicted_box = torch.cat((pred_box_center, pred_box_wh), dim=-1)
      iou_pred_label = calculate_iou(predicted_box, labels[..., 0:4], format="center").detach()
      ...
      object_loss = self.obj_box_loss_func(predictions[..., 4:5] * object_present, (labels[..., 4:5] * object_present) * iou_pred_label)

      pred_xy = self.sigmoid(predictions[..., 0:2])
      pred_wh = predictions[..., 2:4] # Here, the center coordinates should be between 0 and 1 because its ground truth is normalized and at certain(x) % of grid cell size but width height are relative to grid size and divide by anchors.
      pred_box = torch.cat((pred_xy, pred_wh), dim=-1)

      label_xy = labels[..., 0:2]
      label_wh = torch.log(labels[..., 2:4] / large_scale_anchor + 1e-16) # This is according to paper and also ground truth needs to be mapped into the same scale as the predicted
    ...
    if scale == "medium":
      ...
      pred_box_center = self.sigmoid(predictions[..., 0:2])
      pred_box_wh = medium_scale_anchor * torch.exp(predictions[..., 2:4])
      predicted_box = torch.cat((pred_box_center, pred_box_wh), dim=-1)
      iou_pred_label = calculate_iou(predicted_box, labels[..., 0:4], format="center").detach()
      ...
      label_xy = labels[..., 0:2]
      label_wh = torch.log(labels[..., 2:4] / medium_scale_anchor + 1e-16)
      label_box = torch.cat((label_xy, label_wh), dim=-1)
    ...
    if scale == "small":
      ...
      pred_box_center = self.sigmoid(predictions[..., 0:2])
      pred_box_wh = small_scale_anchor * torch.exp(predictions[..., 2:4])
      predicted_box = torch.cat((pred_box_center, pred_box_wh), dim=-1)
      iou_pred_label = calculate_iou(predicted_box, labels[..., 0:4], format="center").detach()
      ...
      label_xy = labels[..., 0:2]
      label_wh = torch.log(labels[..., 2:4] / small_scale_anchor + 1e-16)
      label_box = torch.cat((label_xy, label_wh), dim=-1)
