...
class ConvLayer(nn.Module):
  ...
  def __init__(self, input_channels:int, output_channels:int, kernel:int, activation_func:str="leakyrelu", stride:int=1):
    ...
    if activation_func == "relu":
      ...
      act_fn = nn.LeakyReLU(negative_slope=0.1015625, inplace=True) # In case of activation function used the default is "LeakyReLU"
    
    c_padding = (kernel-1)//2  # This padding is 0 when the stride is '2' which means it will be 0 when the feature map's size has to be reduced. 
    self.stride = stride
